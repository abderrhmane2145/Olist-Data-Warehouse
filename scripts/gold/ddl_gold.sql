...
ON  s.seller_state = g.geolocation_state
...
FROM silver.olist_orders o
	...
	LEFT JOIN 
			(SELECT 
			   ...
			   (SELECT TOP 1 payment_type 
					 FROM silver.olist_order_payments p2 
					 WHERE p2.order_id = p1.order_id 
					 ORDER BY payment_value DESC) AS payment_type
			 FROM silver.olist_order_payments p1
	...
	LEFT JOIN 
				(SELECT
					  ...
					  MAX(shipping_limit_date) AS shipping_date
				 FROM silver.olist_order_items
				 GROUP BY order_id, product_id, seller_id) oi 
